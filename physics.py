...
# Структура данных для передачи состояния шарика
@dataclass
class BallState:
    """Состояние шарика — передаётся в модуль проектора."""
    x: float          # позиция по X (пиксели)
    y: float          # позиция по Y (пиксели)
    vx: float         # скорость по X
    vy: float         # скорость по Y
    angle: float      # угол вращения (радианы)
    radius: int       # радиус шарика (пиксели)
